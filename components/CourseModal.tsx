...
export const CourseModal: React.FC<CourseModalProps> = ({ course, onClose, currentLang, onToggleLang }) => {
  ...
  return (
    <div className={`fixed inset-0 z-50 flex flex-col transition-all duration-500 ease-out ${isVisible ? 'translate-y-0 opacity-100' : 'translate-y-8 opacity-0'} font-sans`} style={{ backgroundColor: theme.bg }} dir={currentLang === 'he' ? 'rtl' : 'ltr'}>
      <div className="w-full px-4 md:px-6 py-4 md:py-5 flex justify-between items-center backdrop-blur-md sticky top-0 z-40 bg-white/30" style={{ borderBottom: `1px solid ${theme.accent}20` }}>
        <button onClick={() => { setIsVisible(false); setTimeout(onClose, 500); }} className="group flex items-center gap-2 text-slate-800 hover:text-slate-600 transition-colors">
          <X className="w-5 h-5 md:w-6 md:h-6 group-hover:rotate-90 transition-transform duration-300" strokeWidth={1.5} />
          <span className="text-[11px] font-semibold uppercase tracking-[0.15em] hidden md:inline">{currentLang === 'he' ? 'סגור' : 'Close'}</span>
        </button>
        <div className="flex items-center gap-3 md:gap-4">
             <button onClick={onToggleLang} className="flex items-center gap-1.5 px-3 py-1.5 rounded-full border border-slate-200 bg-white/60 text-[11px] font-semibold uppercase tracking-wide hover:bg-white transition-colors">
               <Globe className="w-3 h-3" /> {currentLang === 'he' ? 'EN' : 'HE'}
             </button>
             <span className="font-bold text-base md:text-lg">{course.title}</span>
        </div>
      </div>

      <div ref={scrollRef} onScroll={handleScroll} className="flex-grow overflow-y-auto no-scrollbar scroll-smooth">
        <div className="max-w-4xl mx-auto px-5 md:px-6 pb-24 md:pb-32">

            <section className="min-h-[75vh] md:min-h-[80vh] flex flex-col justify-center items-center text-center pt-16 md:pt-20">
                <div className="inline-flex px-4 py-2 rounded-full border text-[11px] font-semibold tracking-[0.2em] uppercase mb-6 md:mb-8" style={{ borderColor: theme.accent, color: theme.text }}>{content.pill}</div>
                <h1 className="text-4xl md:text-7xl lg:text-8xl font-black mb-6 md:mb-8 leading-[0.95] tracking-tight">{content.heroTitle}</h1>
                <p className="text-lg md:text-xl lg:text-2xl max-w-2xl font-normal leading-relaxed text-slate-600">{content.heroSubtitle}</p>
                <div className="mt-10 md:mt-12 w-full max-w-md"><WordCloud /></div>
                <div className="mt-10 md:mt-12 animate-bounce"><ChevronDown className="w-7 h-7 md:w-8 md:h-8 opacity-30" /></div>
            </section>

            {content.shiftTitle && (
              <section className="py-16 md:py-24 grid md:grid-cols-2 gap-8 md:gap-12 border-t" style={{ borderColor: `${theme.accent}20` }}>
                   <div className="space-y-3 md:space-y-4">
                       <span className="text-xs md:text-sm font-semibold tracking-[0.15em] uppercase" style={{ color: theme.accent }}>{content.shiftLabel}</span>
                       <h2 className="text-3xl md:text-4xl font-black leading-tight">{content.shiftTitle}</h2>
                   </div>
                   <div className="space-y-5 md:space-y-6 text-base md:text-lg leading-relaxed text-slate-600">
            ...
            <section className="py-16 md:py-24">
                 <div className="text-center mb-12 md:mb-16">
                     <span className="text-[11px] font-semibold tracking-[0.2em] uppercase mb-3 md:mb-4 block text-slate-500">{content.methodLabel}</span>
                     <h2 className="text-3xl md:text-4xl lg:text-5xl font-black">{content.methodTitle}</h2>
                 </div>
                 <div className="grid md:grid-cols-2 gap-5 md:gap-8">
                     {content.methodSteps.map((step: any, i: number) => (
                         <div key={i} className="p-6 md:p-8 rounded-2xl md:rounded-3xl border border-slate-200/60 bg-white/50 backdrop-blur-sm group hover:bg-white/80 hover:shadow-lg hover:border-slate-200 transition-all duration-300 flex flex-col">
                             <div className="flex justify-between items-start mb-5 md:mb-6">
                                <div className="p-2.5 md:p-3 rounded-xl md:rounded-2xl" style={{ backgroundColor: `${theme.accent}15`, color: theme.accent }}>{step.icon}</div>
                                <span className="text-[10px] font-semibold opacity-40 tracking-wider">{step.label}</span>
                             </div>
                             <h3 className="text-xl md:text-2xl font-bold mb-2 md:mb-3">{step.title}</h3>
                             <p className="text-slate-500 mb-5 md:mb-6 text-sm md:text-base">{step.body}</p>
                             {step.anim && <div className="mt-auto pt-5 md:pt-6 border-t border-slate-100">{step.anim}</div>}
                         </div>
                     ))}
                 </div>
            </section>

            <section className="py-10 md:py-12 border-t flex flex-col md:flex-row items-center gap-8 md:gap-10" style={{ borderColor: `${theme.accent}20` }}>
                <img src="https://lh3.googleusercontent.com/d/1zIWiopYxC_J4r-Ns4VmFvCXaLPZFmK4k=s400?authuser=0" className="w-28 h-36 md:w-32 md:h-40 object-cover rounded-2xl rotate-[-3deg] border-4 border-white shadow-xl" alt="Eyal Izenman" />
                <div className="text-center md:text-left">
                     <h3 className="text-xl md:text-2xl font-bold mb-2">Eyal Izenman</h3>
                     <p className="italic text-slate-500 mb-5 md:mb-6 max-w-sm text-sm md:text-base">
                        {currentLang === 'he' ? '"יצירה היא לא עבודה עם כלים, היא עבודה עם רגש וזמן."' : '"Creation is not about tools, it is about emotion and time."'}
                     </p>
                     <a href="https://wa.me/97236030603" className="inline-flex items-center gap-2.5 md:gap-3 px-6 md:px-8 py-3 md:py-4 rounded-xl text-white font-bold text-base md:text-lg shadow-lg hover:shadow-xl hover:scale-[1.02] transition-all duration-300" style={{ backgroundColor: theme.accent }}>
                        {currentLang === 'he' ? 'דברו איתי על הקורס' : 'Talk to me about the course'} <ArrowRight className="w-4 h-4 md:w-5 md:h-5" />
                     </a>
                </div>
            </section>
        </div>
